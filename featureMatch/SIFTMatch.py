...
class SIFTMatch(object):
    """ match local feature points with SIFT """

    def init(self, pattern, scene, num_kpts = 800, ratio = 0.84, min_match_count = 4):
        """
        Set sift match parameters.
    ...
    def siftCompute(self):
        """
        detect&descript SIFT features and find pattern from the scene image
        ...
        # Create sift object
        sift = cv2.xfeatures2d.SIFT_create(self.num_kpts)

        # Create flann matcher
        matcher = cv2.FlannBasedMatcher(dict(algorithm = 1, trees = 5), {})

        # Detect keypoints and compute keypointer descriptors
        kpts1, descs1 = sift.detectAndCompute(self.pattern, None)
        kpts2, descs2 = sift.detectAndCompute(self.scene, None)
